...
class TestWidthLift:
    ...
    def test_narrow_node_truncates_before_wide_use(self, opcode, left, right, expected):
        capture = builder()
    ...
    def test_same_register_version_uses_existing_widest_binding(self):
        capture = builder()
    ...
    def test_alias_requires_same_location_and_known_version(self, location, version):
        capture = builder()
        capture.instruction(instruction(hx.m_xor, reg(8, 1, 66), reg(location, 4, version)))
        assert len(capture.snapshots) == 2

    def test_reconstruction_extends_without_widening_read(self):
    ...
    def test_unneeded_casts_and_shifts_refuse(self, opcode):
        with pytest.raises(detect.UnsupportedMicrocode):
            builder().instruction(instruction(opcode, reg(8, 1), reg(16, 1)))

    def test_source_wider_than_root_refuses(self):
        with pytest.raises(detect.UnsupportedMicrocode):
            builder(4).instruction(instruction(hx.m_mov, reg(8, 8), size=4))

    def test_nested_operand_and_result_width_mismatch_refuses(self):
        nested = hx.mop_t()
    ...
    def test_builder_cannot_be_reused_across_callbacks(self):
        capture = builder()
    ...
    def test_maximum_uint16_is_valid_version_not_unknown(self):
        capture = builder()
    ...
    def test_legacy_proved_cache_cannot_authorize_width_lift(self, monkeypatch):
        from d810_cobra.rules import cobra_solve as module
        from d810_cobra.solve import SolveResult, SolveStatus
        rule = module.CobraSolveRule(); rule.require_proof = False
        ins = mixed_candidate()
    ...
    def test_narrow_interior_requires_width_domain_with_wide_leaves(self, monkeypatch):
        from d810_cobra.rules import cobra_solve as module
        from d810_cobra.solve import SolveResult, SolveStatus
        from d810_cobra.prove import ProofResult
    ...
    def test_width_proof_abstention_keeps_reason_on_cache_hit(self, monkeypatch, proof, expected_status):
        from d810_cobra.rules import cobra_solve as module
        from d810_cobra.solve import SolveResult, SolveStatus
        from d810_cobra.prove import ProofResult
        rule = module.CobraSolveRule(); rule.require_proof = False
    ...
    def test_saved_roots_python_cython_snapshot_and_ast_parity(self, monkeypatch, request):
        native_mba = _native_mba(request)
    ...
    def test_width_solver_failure_keeps_error_on_cache_hit(self, monkeypatch):
        from d810_cobra.rules import cobra_solve as module
        from d810_cobra.solve import SolveResult, SolveStatus
    ...
    def test_width_expanded_node_budget_boundary(self, narrow, leaves, extra_nodes):
    ...
    def test_width_wide_tree_stops_materializing_at_budget(self):
        capture = builder()
        with pytest.raises(detect.UnsupportedMicrocode, match='width_lift_node_budget'):
            capture.instruction(instruction(hx.m_mov, _balanced_width_input(1024)))
        assert capture._nodes == 200

    def test_width_deep_tree_reports_depth_budget(self):
        op = reg(8, 8)
        for _ in range(30):
